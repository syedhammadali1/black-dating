...
class MessengerEngine extends BaseEngine implements MessengerEngineInterface
{
     ...
     * @return  void
     *-----------------------------------------------------------------------*/
    public function processSendMessage($inputData, $userId, $isBot)
    {
        $transactionResponse = $this->messengerRepository->processTransaction(function () use ($inputData, $userId,$isBot) {
            $userDetails = $this->userRepository->fetchWithProfile($userId);
            // Check if user exists
            if (__isEmpty($userId)) {
            ...
            if (\__isEmpty($messageRequest)) {
                ...
                $initialMessageRequest = [
                    [
                        ...
                        'type' => 9,
                        'from_users__id' => $isBot ? $userId: getUserID(),
                        'to_users__id' => $isBot ? getUserID(): $userDetails->_id,
                        'users__id' => $isBot ? $userId: getUserID(),
                    ...
                    [
                        ...
                        'from_users__id' => $isBot ? $userId: getUserID(),
                        'to_users__id' => $isBot ? getUserID(): $userDetails->_id,
                        'users__id' => $isBot ? getUserID(): $userDetails->_id,
                        'integrity_id' => $initialMessageGeneratedUid
                    ]
                ];
