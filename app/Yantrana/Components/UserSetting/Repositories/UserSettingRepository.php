    ...
    public function fetchFilterData($filterData, $ignoreUserIds, $paginateCount = false)
    {
        ...
        $searchQuery = UserProfile::basicFilter($filterData)
            ...
            if (isset($filterData['state_id'])){
                if($filterData['state_id'] != 'all'){
                    $searchQuery = $searchQuery->where('user_profiles.state_id',$filterData['state_id']);
                }
            }

        $searchQuery = $searchQuery->whereNotIn('users._id', $ignoreUserIds)
            ->select(
                __nestedKeyValues([
                    'users' => [
                        '_id AS user_id',
