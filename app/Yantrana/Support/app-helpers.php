...
/**
 * Fake user/ Bot user ID
 */
if (!function_exists('getFakeUserID')) {
    function getFakeUserID($fakeUserID)
    {
        $user = User::find($fakeUserID);
        $userProfile = UserProfile::where('users__id',$fakeUserID)->where('gender',2)->first();
        if ($user->is_fake == 1 && isset($userProfile)) {
            return true;
        }
        return false;
    }
...
if (!function_exists('getStateID')) {
    function getStateID($getStateID)
    {
        return DB::table('states')->find($getStateID);
    }
}
