...
<nav class="navbar navbar-expand navbar-light bg-dark topbar mb-4 static-top shadow">
    ...
    <ul class="navbar-nav ml-0">
        ...
        <li class="nav-item dropdown no-arrow">
            ...
            if (request()->session()->has('userSearchData')) {
                ...
                $stateWise = $userSearchData['state_id'];
            ...
            <!-- Dropdown - Messages -->
            <div class="dropdown-menu p-3 assadsadsadsad shadow animated--grow-in lw-basic-filter-container" aria-labelledby="searchDropdown">
                <form class="form-inline mr-auto" data-show-processing="true" action="<?= route('user.read.find_matches') ?>">
                    ...
                    <!-- /Looking For -->
                    <!-- State wise -->
                    @isset($statsByUserCountry)
                    <div class="lw-looking-for-container lw-basic-filter-field">
                        <label for="stateWise"><?= __tr('State') ?></label>
                        <select name="state_id" class="form-control" id="stateWise">
                            <option value="all"><?= __tr('All') ?></option>
                            @foreach($statsByUserCountry as $state)
                    ...
                    <div class="lw-distance-location-container lw-basic-filter-field">
                        <label for="distance"><?= __tr('Distance From My Location (__distanceUnit__)', ['__distanceUnit__' => (getStoreSettings('distance_measurement') == '6371') ? 'Miles' : 'Miles']) ?></label>
    ...
    @if(!isPremiumUser())
    <a href="<?= route('payment') ?>" class="btn btn-primary btn-sm" title="<?= __tr('Be Premium User') ?>"><?= __tr('Be Premium') ?></a>
    <!-- /buy premium plans page link -->
    @endif
    <!-- Topbar Navbar -->
